namespace CodeBetter.Json
{
    ...
    public class JsonReader : IDisposable
    {
        ...
        {
            bool hasvalue = AssertAndConsume1(JsonTokens.StringDelimiter);
            if (!hasvalue)
                return null;
        ...
        public virtual DateTime ReadDateTime()
        {
            ...
            return str == null ? DateTime.MinValue : DateTime.Parse(str);
        }
        ...
        //    }
        //}
        protected internal virtual bool AssertAndConsume1(char character)
        {
            char c = Read();
            if (character == JsonTokens.StartArrayCharacter || character == JsonTokens.StartObjectLiteralCharacter || character == JsonTokens.StringDelimiter)
            {
                if (c == 'n')
                {
                    char u = Read();
